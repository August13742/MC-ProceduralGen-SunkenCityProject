...
@jit(nopython=True)
def merge_blocks_fast(terrain_blocks, terrain_remap, city_blocks, city_remap, y_offset, air_idx, target_height):
    """
    ...
    Args:
        ...
        target_height: Fixed output height (e.g., 256 for world height)
    """
    H = target_height if target_height > 0 else max(terrain_blocks.shape[1], city_blocks.shape[1] + y_offset)
...
class ChunkMerger:
    def __init__(self, city_y_start, strategy="underwater", target_height=0):
        """
        Args:
            city_y_start: Y level where city was designed (e.g., 20)
            strategy: Merging strategy - "underwater", "replace", "overlay"
            target_height: Fixed output height (0=auto)
        """
    ...
    def merge_underwater(self, terrain_blocks, terrain_palette, city_blocks, city_palette, cx, cz):
        ...
        result = merge_blocks_fast(
            ...
            y_offset, city_air_idx, self.target_height
        )
...
def main():
    ...
    parser.add_argument("--height", type=int, default=0,
                        help="Output chunk height (0=auto, default: 0)")
    ...
    if args.height > 0:
        print(f"📐 Output height: {args.height} blocks")
    print()
    
    merger = ChunkMerger(city_y_start=args.city_y, strategy=args.strategy, target_height=args.height)
